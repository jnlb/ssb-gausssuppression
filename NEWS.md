...
* Major update of `SuppressKDisclosure()` and related functions.
  ...
  - New parameter `identifying` in `SuppressKDisclosure()` to specify
    information that an intruder may already know.
  - New parameter `sensitive` in `SuppressKDisclosure()` to specify
    information considered unacceptable to disclose.
  - Further specification of identifying–sensitive relations may be provided
    via the `default_targeting()` parameters `targeting_include` and
    `targeting_exclude`.
  - The mechanism underlying these parameters is implemented through a new
    `targeting` parameter in `KDisclosurePrimary()` (see its documentation for
    ...
    `default_targeting()`.
  - The default `singletonMethod` for `SuppressKDisclosure()` has been updated
    (via `PackageSpecs()`) from `"anySumNOTprimary"` to `"anySum0"`.
    See `?SSBtools::GaussSuppression` for details.
